...
(function () {
  ...
  // SoundCloud 公式アイコン（単色版）。fill は currentColor で
  // サイトのオフホワイトを継承 → 暗背景に白、というブランド公式ルールに準拠。
  var SOUNDCLOUD_ICON =
    '<svg class="ev__link-icon" viewBox="0 0 24 24" width="14" height="14" aria-hidden="true" fill="currentColor">' +
  ...
  // Recording リンク（SoundCloud アイコン付き）
  function recordingLink(url) {
    return '<a class="ev__link ev__link--sc" href="' + esc(url) + '" target="_blank" rel="noopener">' +
      SOUNDCLOUD_ICON + '<span>Recording</span></a>';
  }
  ...
  function scheduleItem(ev, isPast) {
    ...
    var links = [];
    if (ev.recording) links.push(recordingLink(ev.recording));
